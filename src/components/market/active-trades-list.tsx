...
export function ActiveTradesList({ teamId }: { teamId: string }) {
    ...
    const handleAccept = async (id: number) => {
        if (!confirm("Confirm trade? This is irreversible.")) return;
        const res = await acceptTradeProposal(id);
        if (res.success) loadTrades();
            ...
            <div className="border rounded-lg p-3 bg-white shadow-sm mb-3">
                ...
                <div className="flex items-center text-xs gap-2 mb-3">
                    <div className="flex-1 bg-blue-50 p-2 rounded">
                        <div className="font-bold text-blue-700 mb-1">YOU GIVE</div>
                        {isProposer ? (
                            <>
                    ...
                    <div className="flex-1 bg-purple-50 p-2 rounded">
                        <div className="font-bold text-purple-700 mb-1">YOU GET</div>
                        {isProposer ? (
                            <>
                                {trade.receiver_player_ids.length} players
                                {trade.receiver_credits > 0 && <span className="block text-[10px]">+ {trade.receiver_credits} cr</span>}
    ...
    if (loading) return <div className="p-4 text-center text-xs text-gray-500">Loading trades...</div>;
    ...
    return (
        <div className="max-h-[60vh] overflow-y-auto">
            ...
            {received.length > 0 && (
                <div className="mb-4">
                    <h4 className="text-xs font-bold text-purple-600 uppercase mb-2">Received Proposals ({received.length})</h4>
            ...
            {sent.length > 0 && (
                <div>
                    <h4 className="text-xs font-bold text-blue-600 uppercase mb-2">Sent Proposals ({sent.length})</h4>
                    {sent.map(t => <TradeCard key={t.id} trade={t} />)}
                </div>
            )}

            {sent.length === 0 && received.length === 0 && (
                <div className="text-center py-8 text-gray-400 text-sm">No active trades.</div>
            )}
        </div>
    );
}
